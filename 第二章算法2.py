...
def algorithm_2_2(F, D, distances, facility_costs, weight):
    # 步骤 1：构造 C 集合
    ...
    while C:
        ...
        if total_cost is not None:
            if max_distance <= 3 * c_bar and total_cost <= weight:
...
for _ in range(num_iterations):
    ...
    model.addConstr(gp.quicksum(facility_costs[i] * y[i] for i in F) <= weight)
    ...
    if model.status == GRB.OPTIMAL:
        ...
        optimal_results.append(optimal_obj_value)
    ...
    result = algorithm_2_2(F, D, distances, facility_costs, weight)
    end_time = time.time()
    ...
    if result:
        ...
        objective_ratio.append(result/optimal_obj_value)
